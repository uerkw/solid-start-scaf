...
import { RouteDefinition, action, cache, createAsyncStore, json } from "@solidjs/router";
...
const getContentFiles = cache(async () => {
  'use server';
  return ((await db.select().from(contentFile).where(eq(contentFile.isMarkedDeleted, false)) ) as SelectContentFile[]) || [];
}, 'notes');
